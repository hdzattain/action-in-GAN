# -*- coding: utf-8 -*-
"""
Created on Sat Dec 11 16:45:50 2021
...
def build_generator(z_dim):
    ...
    return model
def c_gan_generator(z_dim):
    z = Input(shape(z_dim, ))
    label = Input(shape = (1, ), dtype='int32')
    label_embedding = Embedding(num_classes, z_dim, input_length=1)(label)
    label_embedding = Flatten()(label_embedding)
    joined_representation = Multiply()([z, label_embedding])
    generator = build_generator(joined_representation)
...
def build_discriminator(img_shape):
    ...
    model.add(Conv2DTranspose(64, kernel_size=3, strides=1,input_shape = (img_shape[0], img_shape[1], img_shape[2] + 1), padding='same'))
...
discriminator.compile(loss='bianry_crossentropy',
                      optimizerr=Adam(),
                      metrics=['accuracy'])
